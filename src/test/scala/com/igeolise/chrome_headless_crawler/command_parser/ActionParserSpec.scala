...
class ActionParserSpec extends Specification {
  "actionParser" >> {
    ...
    mustParse(NavigateTo(stringValueContinuous), navigateToN, stringValueContinuousTail)
    ...
    mustParse(FindContainingInLastResult(stringValue), findContainingInLastResultN, Some(stringValue))
